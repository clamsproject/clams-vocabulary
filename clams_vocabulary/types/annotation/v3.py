...
class Annotation_v3(Thing_v1, AnnotationTypesBase):
    ...
    alsoKnownAs: ClassVar[list[str]] = [
        "http://mmif.clams.ai/vocabulary/Annotation/v3",
    ]
    ...
    label: Optional[str] = Field(
        None,
        description="A label given to this object by a classifier. The value must be a simple string value of the "
                    "label and must be one of the values defined in the <code>labelset</code> or "
                    "<code>labelsetUri</code> annotation metadata. <br><br> [note] Annotations from a classifier app "
                    "must have this property. [/note]"
    )
